...
def get_args():
    ...
    parser.add_argument("-rm", "--render_mode", default="human_zoom", help="Render mode (human or human_zoom)")
...
def main():
    ...
    poses = np.array(states[args.index][0])
    ...
    env = F1EnvWrapper(env, lambda *_, **__: np.zeros((1,3)), lambda *_, **__: np.zeros(1), lambda *_, **__: 0.0, action_repeat=act_repeat)
    
    control_hz = round(1 / (env.timestep * act_repeat))
